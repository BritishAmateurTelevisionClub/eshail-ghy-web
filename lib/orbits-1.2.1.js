...
/** Additional Functions by Phil Crump 2015 **/
...
function latlonRange(pos1,alt1,pos2,alt2) {
    ...
    var radius = 6371000.0; // Radius of the earth in m
    var e = 8.1819190842622e-2;
    var esq = Math.pow(e, 2);
    
    var N1 = radius / Math.sqrt(1 - esq * Math.pow(Math.sin(lat1), 2));

    var x1 = (N1 + alt1) * Math.cos(lat1) * Math.cos(lon1);
    var y1 = (N1 + alt1) * Math.cos(lat1) * Math.sin(lon1);
    var z1 = ((1 - esq) * N1 + alt1) * Math.sin(lat1);
    ...
    var z2 = ((1 - esq) * N2 + alt2) * Math.sin(lat2);
...
function relativeAzimuth(pos1,alt1,pos2,alt2) {
    ...
    if(bearing < 0)
      return 360 + (bearing * (180.0/Math.PI));
    else
