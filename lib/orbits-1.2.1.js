...
/** Find next AoS
 */
function findAos(lPos, lAlt, elevThreshold) {
    var startTime = (new Date()).getTime(), endTime = startTime + (24 * 60 * 60 * 1000);
    var i;
    for(i=startTime;i<endTime;i+=(2*60*1000)) // 2 minute scan
    {
        future_iss.setDate(new Date(i));
        future_iss.refresh();
        if(relativeElevation(lPos,lAlt,future_iss.orbit.getPosition(),future_iss.orbit.getAltitude()*1000)>elevThreshold)
        {
            var j;
            for(j=i-(2*60*1000);j<i;j+=1000) // 1 second scan
            {
                future_iss.setDate(new Date(j));
                future_iss.refresh();
                if(relativeElevation(lPos,lAlt,future_iss.orbit.getPosition(),future_iss.orbit.getAltitude()*1000)>elevThreshold)
                {
                    // AoS!
...
/** Find LoS during a pass
 */
function findLos(lPos, lAlt, elevThreshold) {
    var startTime = (new Date()).getTime(), endTime = startTime + (24 * 60 * 60 * 1000);
    var i;
    var elevationThreshold = 2;
    for(i=startTime;i<endTime;i+=(1*60*1000)) // 1 minute scan
    {
        future_iss.setDate(new Date(i));
        future_iss.refresh();
        if(relativeElevation(lPos,lAlt,future_iss.orbit.getPosition(),future_iss.orbit.getAltitude()*1000)<elevThreshold)
        {
            var j;
            for(j=i-(1*60*1000);j<i;j+=1000) // 1 second scan
            {
                future_iss.setDate(new Date(j));
                future_iss.refresh();
                if(relativeElevation(lPos,lAlt,future_iss.orbit.getPosition(),future_iss.orbit.getAltitude()*1000)<elevThreshold)
...
/** Additional Functions by Phil Crump 2015-2021 **/
const deg2rad = Math.PI / 180.0;
const rad2deg = 180.0 / Math.PI;
const earth_radius_km = 6.378135e3;
const earth_radius_m = 6.378135e6;
const earth_eccentricity_wgs84 = 8.1819190842622e-2;
const earth_eccentricity_wgs84_sq = Math.pow(earth_eccentricity_wgs84, 2);

function latlonRange(pos1,alt1,pos2,alt2) {
    var lat1 = deg2rad * pos1[0];
    var lon1 = deg2rad * pos1[1];
    var lat2 = deg2rad * pos2[0];
    var lon2 = deg2rad * pos2[1];
    
    var N1 = earth_radius_m / Math.sqrt(1 - earth_eccentricity_wgs84_sq * Math.pow(Math.sin(lat1), 2));

    var x1 = (N1 + alt1) * Math.cos(lat1) * Math.cos(lon1);
    var y1 = (N1 + alt1) * Math.cos(lat1) * Math.sin(lon1);
    var z1 = ((1 - earth_eccentricity_wgs84_sq) * N1 + alt1) * Math.sin(lat1);

    var N2 = radius / Math.sqrt(1 - esq * Math.pow(Math.sin(lat2), 2));

    var x2 = (N2 + alt2) * Math.cos(lat2) * Math.cos(lon2);
    var y2 = (N2 + alt2) * Math.cos(lat2) * Math.sin(lon2);
    var z2 = ((1 - earth_eccentricity_wgs84_sq) * N2 + alt2) * Math.sin(lat2);
    
    /* Return Cartesian Distance in m */
    return Math.sqrt(Math.pow((x1-x2),2) + Math.pow((y1-y2),2) + Math.pow((z1-z2),2));
}

/** Implemented by Phil Crump using maths from earthmaths.py ( Copyright 2012 (C) Daniel Richman; GNU GPL 3 )
 * https://github.com/philcrump/habrotate/blob/master/cli/earthmaths.py
**/
function relativeAzimuth(pos1,alt1,pos2,alt2) {
    var lat1 = deg2rad * pos1[0];
    var lon1 = deg2rad * pos1[1];
    var lat2 = deg2rad * pos2[0];
    var lon2 = deg2rad * pos2[1];
    ...
    return bearing * rad2deg;
}

function relativeElevation(pos1,alt1,pos2,alt2) {
    var lat1 = deg2rad * pos1[0];
    var lon1 = deg2rad * pos1[1];
    var lat2 = deg2rad * pos2[0];
    var lon2 = deg2rad * pos2[1];
    ...
    var ta = earth_radius_m + alt1
    var tb = earth_radius_m + alt2
    var ea = (Math.cos(angle_at_centre) * tb) - ta
    var eb = Math.sin(angle_at_centre) * tb
    var elevation = Math.atan2(ea, eb);
    return elevation * rad2deg;
}

function relativeAngular(A_az_degrees, A_el_degrees, B_az_degrees, B_el_degrees) {
    var az1 = deg2rad * A_az_degrees;
    var el1 = deg2rad * A_el_degrees;
    var az2 = deg2rad * B_az_degrees;
    var el2 = deg2rad * B_el_degrees;

    var a1 = Math.cos((Math.PI / 2) - el1);
    var a2 = Math.cos((Math.PI / 2) - el2);

    var b1 = Math.sin((Math.PI / 2) - el1);
    var b2 = Math.sin((Math.PI / 2) - el2);

    var c = Math.cos(Math.abs(az1 - az2));

    return Math.acos(a1*a2+b1*b2*c) * rad2deg;
}